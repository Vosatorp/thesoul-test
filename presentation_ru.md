...
**Выручка на час произведённого контента (YouTube):**
...
Shorts приносят в **8.5 раз больше выручки на единицу произведённого контента.**

При реалистичном соотношении затрат (Production стоит в 3-5 раз дороже, чем Short) Production всё ещё выгоднее по абсолютному ROI. Но при пропорции 7.5 и выше — Shorts побеждают. Реальные затраты на производство нужно уточнять с командами.
...
Для обычных видео есть чёткая «зона максимальной отдачи». На отметке **8 минут YouTube включает mid-roll рекламу** (реклама в середине ролика) — это официальный порог платформы, и CPM резко растёт. Бакеты 10/15/30 мин — стандартная группировка для анализа длинного контента.
...
Я обучил Gradient Boosting модель, которая предсказывает, станет ли видео долгоживущим. Используются **только 6 признаков, доступных на 7-й день** после публикации (без data leakage):

1. **First 7-Day Views** — абсолютное число просмотров за первую неделю
2. **Watch Time 7D** — время просмотра за первые 7 дней
3. **Video Duration** — длительность видео (известна с момента публикации)
4. **Avg View Duration** — средняя длительность просмотра
5. **Avg % Viewed** — средний процент досмотра
6. **Video Type** — тип контента (Short/Production/Live/Story)

**Качество (5-кратная кросс-валидация):** AUC = 0.71

Модель скромнее по AUC, но честная — не использует метрики, которые станут доступны только через 30+ дней. Главный признак — абсолютное число просмотров за первые 7 дней (47% важности): если видео набирает мало просмотров за неделю, но продолжает расти — это сигнал долгоживущего контента.

**Практическое применение:** видео вышло 7 дней назад, модель оценивает вероятность → помечаем как кандидата в вечнозелёный контент → запускаем продвижение. AUC 0.71 достаточен для скрининга — это фильтр первого этапа, а не финальное решение.
...
# Инсайт 3: Кросс-платформенная публикация ассоциируется с ростом просмотров
...
|-----------|:-------------:|:------------------:|:----:|:-------:|
| Facebook | 56 744 | 117 786 | **+108%** | < 0.0001 |
| Snapchat | 643 876 | 1 568 453 | **+144%** | < 0.0001 |
| YouTube | 15 302 | 6 308 | -59% | 0.28 (не значимо) |
...
⚠️ Selection bias: кросс-постнутые видео изначально имеют 7.7× больше просмотров. Within-channel: эффект подтверждается только в 44% каналов (32 из 73).
...
**Facebook + Snapchat** — самая выгодная пара: средняя совокупная выручка $1 096 за единицу контента (238 примеров).
...
Анализ выявил несколько паттернов, которые стоит проверить глубже:

1. **A/B тесты для подтверждения причинно-следственных связей** — текущие выводы основаны на корреляциях. Для кросс-постинга и длительности нужны контролируемые эксперименты.
2. **ML-скоринг evergreen-контента** — модель (AUC 0.71) может быть улучшена с дополнительными данными (расходы на промо, демографика аудитории, разметка SMM-метода).
3. **Watch time как основная метрика** — замена engagement rate на watch time в дашбордах для более точной оценки контента.
4. **Диверсификация Snapchat** — мониторинг channel_621 и масштабирование практик топ-каналов.
